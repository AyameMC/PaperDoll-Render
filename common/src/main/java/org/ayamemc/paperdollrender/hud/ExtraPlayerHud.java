...
package org.ayamemc.paperdollrender.hud;
...
public class ExtraPlayerHud {
    ...
    private static int getLight(Entity entity, float tickDelta) {
        if (PaperDollRender.CONFIGS.useWorldLight.getValue()) {
            ...
            int min = PaperDollRender.CONFIGS.worldLightMin.getValue();
    ...
    public void render(float partialTicks) {
        if (client.level == null || client.player == null || !PaperDollRender.CONFIGS.enabled.getValue()) return;
        LivingEntity targetEntity = client.level.players().stream().filter(p -> p.getName().getString().equals(PaperDollRender.CONFIGS.playerName.getValue())).findFirst().orElse(client.player);
        if (PaperDollRender.CONFIGS.spectatorAutoSwitch.getValue() && client.player.isSpectator()) {
        ...
        Configs.PoseOffsetMethod poseOffsetMethod = PaperDollRender.CONFIGS.poseOffsetMethod.getValue();
        ...
        if (PaperDollRender.CONFIGS.renderVehicle.getValue() && poseOffsetMethod != Configs.PoseOffsetMethod.FORCE_STANDING && targetEntity.isPassenger()) {
            ...
            performRendering(vehicle,
                    PaperDollRender.CONFIGS.offsetX.getValue() * scaledWidth,
                    PaperDollRender.CONFIGS.offsetY.getValue() * scaledHeight,
                    PaperDollRender.CONFIGS.size.getValue() * scaledHeight,
                    PaperDollRender.CONFIGS.mirrored.getValue(),
                    ...
                    PaperDollRender.CONFIGS.lightDegree.getValue(),
                    partialTicks);
        }
    ...
    private double getPoseOffsetY(LivingEntity targetEntity, float partialTicks, Configs.PoseOffsetMethod poseOffsetMethod) {
        if (poseOffsetMethod == Configs.PoseOffsetMethod.AUTO) {
            ...
            if (targetEntity.isFallFlying()) {
                return PaperDollRender.CONFIGS.elytraOffsetY.getValue() * getFallFlyingLeaning(targetEntity, partialTicks);
                ...
                return PaperDollRender.CONFIGS.swimCrawlOffsetY.getValue();
            } else if (!targetEntity.isVisuallySwimming() && targetEntity.getSwimAmount(partialTicks) > 0) { // for swimming/crawling pose, only smooth the falling edge
                return PaperDollRender.CONFIGS.swimCrawlOffsetY.getValue() * targetEntity.getSwimAmount(partialTicks);
            } else if (targetEntity.isCrouching()) {
                return PaperDollRender.CONFIGS.sneakOffsetY.getValue();
            }
        }
        return 0;
    }
    ...
    private void performRendering(Entity targetEntity, double posX, double posY, double size, boolean mirror,
        ...
        Quaternionf quaternion2 = new Quaternionf()
                .rotateXYZ((float) Math.toRadians(PaperDollRender.CONFIGS.rotationX.getValue()),
                        (float) Math.toRadians(PaperDollRender.CONFIGS.rotationY.getValue()),
                        (float) Math.toRadians(PaperDollRender.CONFIGS.rotationZ.getValue()));
